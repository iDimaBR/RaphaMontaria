...
public class NewFriendConversation implements Prompt {
    ...
    public Prompt acceptInput(ConversationContext ctx, String input) {
        ...
        if(target == player){
            player.sendRawMessage("§cVocê não pode ser adicionado como amigo!");
            return END_OF_CONVERSATION;
        }

        final List<UUID> friends = cache.getFriends();
        friends.add(target.getUniqueId());
        repository.update(cache);

        player.sendRawMessage("§aVocê permitiu o jogador §f" + target.getName() + "§a montar seus cavalos.");
        return END_OF_CONVERSATION;
    }
}
